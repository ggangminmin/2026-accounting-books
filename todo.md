...
- [x] 대시보드 카드 레이아웃 변경
  - [x] 사용률 카드 제거
  - [x] 회비 카드 추가 (초기값: 496,000원)
  - [x] 찬조금 카드 추가 (초기값: 11,133원)
  - [x] 항목별 카드에 초기 금액 표시 추가 (2026-01-10)
    - 일반 예산: "초기 3,200,000원"
    - 회비: "초기 496,000원"
    - 찬조금: "초기 11,133원"
- [x] 숫자 포맷팅 시스템
...
- [x] 지출 추가 기능 강화
  ...
  - [x] 개별 영수증 삭제 버튼
- [x] 예산 복원 로직 구현
  - [x] 지출 수정 시작 시 기존 출처 예산 복원
  - [x] 지출 수정 저장 시 새 출처에서 차감
  - [x] 지출 수정 취소 시 예산 재차감
  - [x] Supabase 실시간 동기화
...
- [x] Supabase 데이터베이스 마이그레이션
  ...
  - [x] `settings` 테이블 생성
  - [x] RLS 정책 설정
- [x] Service Worker 캐싱 최적화
  - [x] POST 요청 캐싱 제외
  - [x] chrome-extension URL 캐싱 제외
  - [x] Supabase API 캐싱 제외
- [x] 에러 처리 강화
...
### 대시보드
- ✅ 총 예산, 총 지출, 남은 예산 카드 표시
- ✅ 회비 (초기 496,000원), 찬조금 (초기 11,133원) 카드 표시
- ✅ 모든 금액에 천 단위 콤마 적용
- ✅ 카테고리별 지출 차트 표시
